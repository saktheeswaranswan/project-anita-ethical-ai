...
class BallSimulation:
    ...
    def add_anita_to_histogram(self, bin_index):
        ...
        y = self.canvas_height - 50 - (self.bins[bin_index] * 40)  # Adjust for the histogram area and size of Anita image
    ...
    def draw_bins(self):
        ...
        for i, count in enumerate(self.bins):
            ...
            y0 = self.canvas_height - 250  # Start from bottom of the canvas
            y1 = y0 - (count * 4)  # Scale factor for the bin height
            color = f'#{random.randint(0, 0xFFFFFF):06x}'  # Random color for each bin
            self.canvas.create_rectangle(x0, y0, x1, y1, fill=color, outline='black', tags="bin")

    def save_histogram_as_png(self):
        # Create a histogram plot
        plt.figure(figsize=(11.7, 16.5))  # A0 size in inches
