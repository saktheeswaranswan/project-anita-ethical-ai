...
class BallSimulation:
    def __init__(self, master, num_balls, num_bins):
        ...
        # Create canvases for displaying factors
        self.left_factor_canvas = tk.Canvas(master, width=300, height=200, bg='lightgrey')
        self.left_factor_canvas.pack(side=tk.LEFT)
        
        self.right_factor_canvas = tk.Canvas(master, width=300, height=200, bg='lightgrey')
        self.right_factor_canvas.pack(side=tk.RIGHT)

        # Factors affecting development
        self.factors = self.create_factors()
        self.left_factors = self.factors[:25]  # First 25 factors
        self.right_factors = self.factors[25:50]  # Next 25 factors
    ...
    def show_factors(self):
        self.left_factor_canvas.delete("factor")
        self.right_factor_canvas.delete("factor")
        for index, factor in enumerate(self.left_factors):
            if self.blink_state:
                self.left_factor_canvas.create_text(10, 10 + index * 15, anchor='nw', text=factor, tags="factor")
        for index, factor in enumerate(self.right_factors):
            if self.blink_state:
                self.right_factor_canvas.create_text(10, 10 + index * 15, anchor='nw', text=factor, tags="factor")
