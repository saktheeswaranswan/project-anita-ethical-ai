...
class BallSimulation:
    ...
    def create_red_balls(self):
        ...
        num_columns = 8  # Fixed number of columns
        x_start = (self.canvas_width // 2) - (num_columns * 20) // 2
        y_start = (self.canvas_height // 2) - (num_rows * 20) // 2
    ...
    def blink_ball(self):
        original_color = self.canvas.itemcget(self.blue_ball_id, 'image')
        for _ in range(5):  # Blink 5 times
            self.canvas.itemconfig(self.blue_ball_id, state='hidden')
            self.master.update()
            time.sleep(0.1)
            self.canvas.itemconfig(self.blue_ball_id, state='normal')
            self.master.update()
            time.sleep(0.1)
